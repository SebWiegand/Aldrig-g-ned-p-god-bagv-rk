...
# ============================================================
# Minimum singular value required for a topic to be kept.
# Set to None to disable filtering.
MIN_TOPIC_SINGULAR_VALUE = 50
...
USE_BIGRAMS = True
BIGRAM_MIN_COUNT = 50   # bigram must appear at least this many times in the corpus
...
def main():
    ...
    # ------------------------------------------------------------
    # ROLLING-WINDOW RUN (text analytics only)
    # ------------------------------------------------------------
    WINDOW_START = 2020
    WINDOW_END = 2024
    ...
    for fname, rng in page_ranges.items():
        bank, year = _parse_bank_year_from_filename(fname)
        if year is not None and WINDOW_START <= year <= WINDOW_END:
            page_ranges_year[fname] = rng

    if not page_ranges_year:
        raise RuntimeError(
            f"No PDFs matched rolling window {WINDOW_START}-{WINDOW_END}. "
            "Check filename parsing and page_ranges keys."
        )

    run_label = f"{WINDOW_START}_{WINDOW_END}"

    print(f"Running analysis on rolling window {WINDOW_START}-{WINDOW_END}")
    ...
    summary_path = os.path.join(out_folder, f"extraction_summary_{run_label}_V1.csv")
    ...
    # ------------------------------------------------------------
    # Optional filtering: keep only topics with singular value >= threshold
    # ------------------------------------------------------------
    if MIN_TOPIC_SINGULAR_VALUE is not None:
        singular_values_df = tf_results["singular_values_df"].copy()

        # detect column names robustly
        sv_col = None
        for c in ["singular_value", "Singular Value", "value", "sv", "leading_singular", "second_singular"]:
            if c in singular_values_df.columns:
                sv_col = c
                break

        topic_col = None
        for c in ["topic", "topic_id", "topic_name", "Topic", "cluster", "cluster_id"]:
            if c in singular_values_df.columns:
                topic_col = c
                break

        if sv_col is None or topic_col is None:
            raise ValueError(
                "Could not find topic/singular-value columns in singular_values_df. "
                f"Found columns: {list(singular_values_df.columns)}"
            )

        singular_values_df[sv_col] = pd.to_numeric(singular_values_df[sv_col], errors="coerce")

        keep_topics = set(
            singular_values_df.loc[
                singular_values_df[sv_col] >= MIN_TOPIC_SINGULAR_VALUE,
                topic_col
            ].astype(str)
        )

        total_topics_before = singular_values_df[topic_col].astype(str).nunique()
        total_topics_after = len(keep_topics)

        # 1) singular values
        tf_results["singular_values_df"] = tf_results["singular_values_df"][
            tf_results["singular_values_df"][topic_col].astype(str).isin(keep_topics)
        ].copy()

        # 2) topic importances
        if not tf_results["topic_importances_df"].empty:
            imp_topic_col = None
            for c in ["topic", "topic_id", "topic_name", "Topic", "cluster", "cluster_id"]:
                if c in tf_results["topic_importances_df"].columns:
                    imp_topic_col = c
                    break
            if imp_topic_col is not None:
                tf_results["topic_importances_df"] = tf_results["topic_importances_df"][
                    tf_results["topic_importances_df"][imp_topic_col].astype(str).isin(keep_topics)
                ].copy()

        # 3) topic words
        if not tf_results["topics_words_df"].empty:
            tw_topic_col = None
            for c in ["topic", "topic_id", "topic_name", "Topic", "cluster", "cluster_id"]:
                if c in tf_results["topics_words_df"].columns:
                    tw_topic_col = c
                    break
            if tw_topic_col is not None:
                tf_results["topics_words_df"] = tf_results["topics_words_df"][
                    tf_results["topics_words_df"][tw_topic_col].astype(str).isin(keep_topics)
                ].copy()

        # 4) first_doc_topics_df -> keep only matching topic_loading_* columns
        if not tf_results["first_doc_topics_df"].empty:
            fd_cols = [c for c in tf_results["first_doc_topics_df"].columns if c.startswith("topic_loading_")]
            keep_fd_cols = [c for c in fd_cols if c.replace("topic_loading_", "") in keep_topics]
            base_fd_cols = [c for c in tf_results["first_doc_topics_df"].columns if not c.startswith("topic_loading_")]
            tf_results["first_doc_topics_df"] = tf_results["first_doc_topics_df"][base_fd_cols + keep_fd_cols].copy()

        # 5) second_doc_topics_df -> same logic
        if "second_doc_topics_df" in tf_results and not tf_results["second_doc_topics_df"].empty:
            sd_cols = [c for c in tf_results["second_doc_topics_df"].columns if c.startswith("topic_loading_")]
            keep_sd_cols = [c for c in sd_cols if c.replace("topic_loading_", "") in keep_topics]
            base_sd_cols = [c for c in tf_results["second_doc_topics_df"].columns if not c.startswith("topic_loading_")]
            tf_results["second_doc_topics_df"] = tf_results["second_doc_topics_df"][base_sd_cols + keep_sd_cols].copy()

        print(
            f"Applied MIN_TOPIC_SINGULAR_VALUE={MIN_TOPIC_SINGULAR_VALUE}. "
            f"Topics kept: {total_topics_after}/{total_topics_before}"
        )
    ...
    if not tf_results["first_doc_topics_df"].empty:
        kept_topic_cols = [c for c in tf_results["first_doc_topics_df"].columns if c.startswith("topic_loading_")]
        print(f"Final number of extracted topics after singular-value filtering: {len(kept_topic_cols)}")

    print("\n=== Saving results ===")
    tf_results["first_doc_topics_df"].to_csv(
        os.path.join(out_folder, f"first_doc_topics_{run_label}.csv"),
        index=False,
    )
    ...
    if not tf_results["second_doc_topics_df"].empty:
        tf_results["second_doc_topics_df"].to_csv(
            os.path.join(out_folder, f"second_doc_topics_{run_label}.csv"),
            index=False,
        )

    tf_results["topics_words_df"].to_csv(
        os.path.join(out_folder, f"topics_words_{run_label}.csv"),
        index=False,
    )
    tf_results["singular_values_df"].to_csv(
        os.path.join(out_folder, f"singular_values_{run_label}.csv"),
        index=False,
    )
    tf_results["topic_importances_df"].to_csv(
        os.path.join(out_folder, f"topic_importances_{run_label}.csv"),
        index=False,
    )

    print("\nPipeline finished ✓")
    print(f"Window run completed: {WINDOW_START}-{WINDOW_END}")
